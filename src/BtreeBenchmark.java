...
public class BtreeBenchmark {
    ...
    public static void bench(int size,String sent) {
        Integer[] key = new Integer[size];
        String[] value = new String[size];
        long[] time=new long[10];
        long[] time2=new long[10];
    for(int i = 0; i < 10; i++){
        IBTree<Integer,String> dummy = new BTree<Integer,String>(3);
        for (int j = 0; j < size; j++) {
             key[i]=(int) Math.floor(Math.random() * (429496729 - 1 + 1) + 1);
    ...
    FileWriter csvWriter = new FileWriter("../"+sent+".csv");
      ...
      csvWriter.append("insertion time");
      for(int i=0 ;i<10;i++){
      ...
      csvWriter.append("deletion time");
      for(int i=0 ;i<10;i++){
